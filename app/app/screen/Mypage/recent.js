...
const Recent = (props) => {
    ...
    async function get_datalist() {
        setLoading(false);
        await Axios.get('&act=goods&han=get_recent&mem_idx='+props.baseData.userData.idx,
            (response) => {
    ...
    if(loading) {
        return (
            <Layout havebottom={false} havetop={true} havnoback={true} havebtn1={false} havebtn2={false} toptext={"최근 본 상품"} navigation={props.navigation}>
                <View style={[glSt.px24]}>
                    {datalist.length == 0 ?
                    <View style={[glSt.alcenter,glSt.jucenter,glSt.pt54]}>
                        <Text style={[glSt.cC0BCB6,glSt.text14m]}>최근 본 상품이 없어요.</Text>
                    </View>
                    :
                    <ScrollView>
