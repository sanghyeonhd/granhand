...
$q = "SELECT [FIELD] FROM shop_board WHERE isdel='N'";
			...
			<div class="panel-content">
				...
				<tR>
					...
					<td>
						...
						<select name='se_boardid'>
							<option value=''>게시판선택</option>
							<?
							$q = "SELECT * FROM shop_board_conf";
							$r = mysqli_query($connect,$q);
							while($row = mysqli_fetch_array($r))	{
								if( $se_boardid == $row[board_id] ) echo "<option value='$row[board_id]' selected>$row[board_name]</option>";
								else								echo "<option value='$row[board_id]'>$row[board_name]</option>";
							}
							?>
						</select>
						</div>
...
<div class="row">
	<div class="col-md-12">
		<div class="panel">
			<div class="panel-header">
				<h3><i class="fa fa-table"></i> 게시글목록</h3>
			</div>
			<div class="panel-content">
...
for($is=0;$is<sizeof($data);$is++)	{
	...
	$ar_board_conf = sel_query_all("shop_board_conf"," WHERE board_id='$row[boardid]'");
	if($row['cates']!='')	{
		$ar_cates = sel_query_all("shop_board_cate"," WHERE index_no='$row[cates]'");
	}
?>
				...
				<tr>
					...
					<td>
						...
						<a href="#none" onclick="event.preventDefault(); delchk(this,<?=$row['index_no'];?>,'board','del_board')" class="btn btn-sm btn-primary">삭제</a>
					</td>
				</tr>
<?php
}
