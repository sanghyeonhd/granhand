...
$q = "SELECT [FIELD] FROM shop_newmarketdb INNER JOIN shop_newbasket ON shop_newmarketdb.idx=shop_newbasket.market_idx WHERE pdan='3'";
	...
	<div class="col-md-12">
		<div class="panel">
			<div class="panel-header">
				<h3><i class="fa fa-table"></i> 수거중목록</h3>
			</div>
...
for($is=0;$is<count($data);$is++){
	...
	$ar_goods = sel_query("shop_goods","gname,simg1"," WHERE idx='$row[goods_idx]'");
?>
				<tr>
					<td ><input type='checkbox' name='index' value='<?=$row['idx'];?>'></td>
					<td >
						<?=$row['sdate'];?> <?=$row['shour'];?>
						<? if($row['indate']!='') { echo "<br />(".$row['indate']." ".$row['intime'].")";	}?>
					</td>
					<td >
						<a href="javascript:MM_openBrWindow('popup.php?code=order_nview&idx=<?=$row[idx];?>','order<?=$row[idx];?>','scrollbars=yes,width=1150,height=900,top=0,left=0');"><? echo date("Ym",$row['orderno'])."-".$row['idx']; ?></a>
					</td>
					<td ><?=$row['name'];?></td>
					<td></td>
