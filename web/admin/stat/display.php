...
<div class="row">
	<div class="col-md-12">
		<div class="panel">
			<div class="panel-header">
				<h3><i class="fa fa-table"></i> 해상도</h3>
			</div>
			<div class="panel-content">
				 <?php
          	...
          	$qry = "SELECT @rownum := @rownum +1 AS rownum, a. * 
											FROM (
											SELECT display, COUNT( id ) AS count
											FROM tb_data
...
if ($row != 0 )
{
	while( $data = mysql_fetch_array($result, MYSQL_ASSOC)) {
		$num				= $data["rownum"];
		$dis_name 		= $data["display"];
		$count			= $data["count"];
		
		if ($dis_name == "") {
			$dis_name = "알수없음(Unknown)";
		}
	          	
  	echo "<tr>";
  	echo "	<td class='first'>".$num."</td>";
  	echo "	<td style='background-color:#EEB4E9;'>".$dis_name."</td>";
  	echo "	<td id='dis_avg_".$num."'>NaN</td>";
  	echo "</tr>";
  	echo "<input id='dis_cnt_".$num."' type='hidden' value='".$count."' />";
