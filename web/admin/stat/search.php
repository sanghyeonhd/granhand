...
$q = "SELECT [FIELD] FROM shop_search_keyword WHERE 1";
if($se_sdate)	{
	$q = $q . " AND LEFT(wdate,10)>='$se_sdate'";
}
if($se_edate)	{
	$q = $q . " AND LEFT(wdate,10)<='$se_edate'";
}
$q = $q . " group by keyword";
...
//데이터쿼리
$_sql = str_replace("[FIELD]", "count(idx) as ks,keyword", $q);
...
<div class="row">
	<div class="col-md-12">
		<div class="panel">
			<div class="panel-header">
				<h3><i class="fa fa-table"></i> 검색키워드목록 - 총 : <?=number_format($total_record);?></h3>
			</div>
			<div class="panel-content">
