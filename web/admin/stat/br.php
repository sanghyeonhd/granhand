...
<div class="row">
	<div class="col-md-12">
		<div class="panel">
			<div class="panel-header">
				<h3><i class="fa fa-table"></i> 브라우저</h3>
			</div>
			<div class="panel-content">
				 <?php
          	...
          	$qry = "SELECT @rownum := @rownum +1 AS rownum, a. * 
											FROM (
											SELECT browser, browser_ver, COUNT( id ) AS count
											FROM tb_data
...
if ($row != 0 )
{
	while( $data = mysql_fetch_array($result, MYSQL_ASSOC)) {
		$num				= $data["rownum"];
		$br_name 		= $data["browser"];
		$br_ver			= $data["browser_ver"];
		$count			= $data["count"];
		
		if ($br_name != "") {
			$br_full = $br_name." ".$br_ver;
		}
		else {
			$br_full = "알수없음(Unknown)";
		}
	          	
  	echo "<tr>";
  	echo "	<td class='first'>".$num."</td>";
  	echo "	<td style='background-color:#EEB4E9;'>".$br_full."</td>";
  	echo "	<td id='br_avg_".$num."'>NaN</td>";
  	echo "</tr>";
  	echo "<input id='br_cnt_".$num."' type='hidden' value='".$count."' />";
