...
<div class="row">
	<div class="col-md-12">
		<div class="panel">
			<div class="panel-header">
				<h3><i class="fa fa-table"></i> 블로그/커뮤니티</h3>
			</div>
			<div class="panel-content">
				 <?php
          	
          	$qry = "SELECT @rownum := @rownum +1 AS rownum, aa. * 
											FROM (
												SELECT a.rf_sub, a.rf_domain, COUNT( a.id ) AS count
												FROM (
													SELECT id, rf_sub, rf_domain
													FROM tb_data
													WHERE reg_date >=  '".$sdate."'
													AND reg_date <  DATE_FORMAT('".$edate."' +INTERVAL 1 DAY,'%Y-%m-%d')
												) AS a
												WHERE rf_sub LIKE  '%blog%'
												OR rf_sub LIKE  '%cafe%'
												OR rf_domain =  'facebook.com'
												GROUP BY a.rf_sub, a.rf_domain
												ORDER BY count DESC
												) AS aa , (SELECT @rownum :=0) r
											LIMIT 1000";
...
if ($row != 0 )
{
	while( $data = mysql_fetch_array($result, MYSQL_ASSOC)) {
  	...
  	echo "	<td style='background-color:#EEB4E9;'>".$rf_full."</td>";
...
?>					
		<tbody>
		<tr>
			<td class="first" colspan="4"><p style="color:red;">데이터 없음</p></td>
		</tr>
<?php } ?>
		</tbody>
	</table>
