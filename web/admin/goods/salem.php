...
if($mode=='w')
{
	...
	if($value['sale_t']=='3')	{
		$value['sale_t_arr'] = $_POST['sale_t_sub_3'];
	}
	if($value['sale_t']=='4')	{
		$value['sale_t_arr'] = $_POST['sale_t_sub_4'];
	}
...
function set_subs()	{
	...
	if($("#sale_t option:selected").val()=='3')	{
		$("#sale_t_sub_3").show();
	}
	if($("#sale_t option:selected").val()=='4')	{
		$("#sale_t_sub_4").show();
	}
...
<select class="uch" name='pid' id="pid">
						...
						if($ar_data[fid])
						{
							$q = "Select * from shop_config where fid='$ar_data[fid]' and site_mobile!='O' order by index_no asc";
							$st = $pdo->prepare($q);
$st->execute();	
							while($row = $st->fetch())
							{	
...
<tr id="stype2_2">
	<th>할인대상상품</th>
	<td>
		<select name='sale_t' id='sale_t' onchange="set_subs();">
		...
		<option value='4' <? if($ar_data[sale_t]=='4'){ echo "selected";	}?>>기획전(이벤트)상품</option>
		</select>
	</td>
</tr>

<tr id="sale_t_sub_3" <? if($ar_data['sale_t']!='3') { ?>style="display:none;"<?}?>>
	<th>할인카테고리선택</th>
	<td>
		<select class="uch" name='sale_t_sub_3'>
...
$st->execute();	
						while($row = $st->fetch())	{
							$se = "";
							if($ar_data['sale_t_arr']==$row['catecode'])	{
								$se = "selected";
							}
...
<tr id="sale_t_sub_4" <? if($ar_data['sale_t']!='4') { ?>style="display:none;"<?}?>>
	<th>기획전선택</th>
	<td>
		<select class="uch" name='sale_t_sub_4'>
		<option value=''>기획전선택</option>
		<?
		$q= "select * from shop_event where isdel='N' and listimg!='' AND edate>='".date("Y-m-d H:i:s")."' and isshow='Y'";
		$st = $pdo->prepare($q);
$st->execute();	
		while($row = $st->fetch())	{
			
			$se = "";
			if($row['index_no']==$ar_data['sale_t_arr'])	{
				$se = "selected";
			}
			echo "<option value='$row[index_no]' $se>$row[subject]</option>";
		}
		?>
		</select>
	</td>
...
<tr>
	...
	<td>
		<div class="form-inline">
		<input type='radio' name='saleops' value='1'  <? if($ar_data[saleops]=='1') { echo "checked";	}?>>옵션가까지할인 <input type='radio' name='saleops' value='2' <? if($ar_data[saleops]=='2') { echo "checked";	}?>>옵션가할인안함
		</div>
	</td>
</tr>
<?}?>
