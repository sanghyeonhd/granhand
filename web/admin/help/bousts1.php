...
if($mode=='w')
{
	...
	for($i=0;$i<sizeof($ar_index);$i++)
	{
		if($ar_index[$i]!='')
		{
			...
			update("shop_newmarketdb_accounts",$value," where index_no='$ar_index[$i]'");
			unset($value);
		}
	}
...
if($mode=='w2')
{
	...
	for($i=0;$i<sizeof($ar_index);$i++)
	{
		if($ar_index[$i]!='')
		{
			$ar_in = sel_query_all("shop_newmarketdb_accounts"," where index_no='$ar_index[$i]'");
			...
			else
			{
				...
				update("shop_newmarketdb_accounts",$value," where index_no='$ar_in[up_idx]'");
				unset($value);
				...
				$ar_ma = sel_query_all("shop_newmarketdb"," where index_no='$ar_in[market_idx]'");
				$ar_kinit = sel_query_all("shop_config"," where index_no='$ar_ma[pid]'");
...
function show_memo(ids,mk,cou)
{
	...
	{	
		...
		fos1.location.href='./frame_data/o_memos2.php?frs=fr1'+ids+'&index_no='+mk;
		fos2.location.href='./frame_data/o_account2.php?frs=fr2'+ids+'&index_no='+mk;
	}
	else
	{	
...
<tbody>
<?
$q= "select count(shop_newmarketdb_accounts.index_no),sum(shop_newmarketdb_accounts.account) from shop_newmarketdb_accounts,shop_newmarketdb where tbtype='O' and shop_newmarketdb_accounts.buymethod='B' and shop_newmarketdb_accounts.market_idx=shop_newmarketdb.index_no";
if($dans=='1')
...
$q= "select count(shop_newmarketdb_accounts.index_No) from shop_newmarketdb_accounts,shop_newmarketdb where tbtype='O' and shop_newmarketdb_accounts.buymethod='B' and shop_newmarketdb_accounts.market_idx=shop_newmarketdb.index_no";
if($dans=='1')
{	$q = $q . " and incdate='' and checkd=''";	}
if($dans=='2')
{	$q = $q . " and incdate='' and checkd!=''";	}
...
$q = "select shop_newmarketdb_accounts.*,shop_newmarketdb.pid,shop_newmarketdb.fid from shop_newmarketdb_accounts,shop_newmarketdb where tbtype='O' and shop_newmarketdb_accounts.buymethod='B' and shop_newmarketdb_accounts.market_idx=shop_newmarketdb.index_no";
if($dans=='1')
{	$q = $q . " and incdate='' and checkd=''";	}
if($dans=='2')
{	$q = $q . " and incdate='' and checkd!=''";	}
...
while($row = $st->fetch())
{
	$ar_m = sel_query("shop_newmarketdb","index_no,name,orderno"," where index_no='$row[market_idx]'");
	$co = "";
	if(!($cou%2)) $co = "gray";
...
<tr  class='<?=$co;?>'>
<td class="first"><input type='checkbox' name='index' value='<?=$row[index_no];?>'>
<td><?=$article_num;?></td>
<td>
	<?
...
<td><!-- <? if($dan!='1'){?>  <span class="btn_white_xs"><a href="javascript:MM_openBrWindow('./module/change_bank.php?index_no=<?=$row[index_no];?>','cb','scrollbars=yes,width=500,height=200,top=0,left=0');">수정</a></span><?}?> --></td>
</tr>
<?php
	$article_num--;
	$cou++;
